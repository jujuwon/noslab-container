...
echo `mpstat 1 30 >> mpstat_log1.txt &`
echo `docker run --rm --name nos_test1 -d nos_dummy 1`

sleep 30s

echo `mpstat 1 30 >> mpstat_log2.txt &`
for i in 1 2
do
...
echo `mpstat 1 30 >> mpstat_log4.txt &`
for i in {1..4}
do
    echo `docker run --rm --name nos_test$i -d nos_dummy $i`
done
...
echo `mpstat 1 30 >> mpstat_log8.txt &`
for i in {1..8}
do
    echo `docker run --rm --name nos_test$i -d nos_dummy $i`
done
...
echo `mpstat 1 30 >> mpstat_log16.txt &`
for i in {1..16}
do
    echo `docker run --rm --name nos_test$i -d nos_dummy $i`
done
